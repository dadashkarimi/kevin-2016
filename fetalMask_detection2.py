...
import sys
sys.path.append('/autofs/space/bal_004/users/jd1677/kevin-2016/opencv/local_install/usr/local/lib64')
...
img = irtk.imread(filename, dtype="float32").saturate(1,99).rescale()
...
detections = detect_mser(
    filename,
    ga,
    vocabulary,
    mser_detector,
    1.0,
    output_folder=output_dir,  # Optional: Set other arguments if needed
    DEBUG=args.debug,
    return_image_regions=True  # Optional: Modify if needed
)
# print(detections)
# print len(detections[0])
detections = [ai for ai in detections if len(ai)!=0 ]
print "Fit box with RANSAC"
center, selection = ransac_ellipses( detections,
                                     ga)
